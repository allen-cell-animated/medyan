...
\textbullet~Change g++ directory \texttt{CXX} = /usr/bin/g++ -std=c++11 to
 	    CXX = /your g++ directory/g++ -std=c++11 or		CXX = g++ -std=c++11\\
You may need to load g++ module manually.\\
 \textbullet~Change library directory \texttt{LDLIBS} = -L/usr/local/lib/ -lboost $\textunderscore$ system -lgsl to 
		LDLIBS =-L/usr/local/lib/ -lboost$\textunderscore$system -L/your boost library directory/lib/ 
		For some reasons, -lgsl may cause error and remove this command does not affect installation. \\
\textbullet~Change third line in \texttt{CPPFLAGS} -I/usr/local/include -I/usr/include/boost to
		-I/your library header directory/include -I/your library header directory/include/boost 
