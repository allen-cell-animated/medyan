...
\section {Introduction}
 ...
 processes. His approach and model, named the \textit{Mechano-chemical Dynamics of Active Networks,
...
The following mechanical parameters can be set. It should be noted that the number of parameters for each force field must match the number of chemical species of that type. Force field constant units are dependent on the potential used. For more information on force fields used in the MEDYAN3 model, see (*paper*). If a force field type is left blank, that force field will not be included in the simulation.

\begin{table} [!ht]
\centering
\begin{tabular}{|L{4.65cm}|C{4cm}|L{7cm}|}  
\hline
 \textbf{Parameter} & \textbf{Value type} & \textbf{Description} \\
 \hline
...
The following chemical parameters can be set. The number of chemical species of each type must match the chemistry input file, as well as the number of mechanical parameters for each force field. All chemical parameters must be set unless otherwise noted in the description. For more information on chemical algorithms, see (*paper*)

\begin{table} [!ht]
\centering
\begin{tabular}{|L{4.9cm}|C{3.8cm}|L{6.6cm}|}  
\hline
 \textbf{Parameter} & \textbf{Value type} & \textbf{Description} \\
 \hline
  ...
  \hline
    NUMMOTORSPECIES & int & Number of Myosin II motor species in system \\
  \hline
...
\hline
 ...
 \hline
  ...
  \hline
  MDULENGTH & double & Characteristic length for Myosin II unbinding rate change \\
  \hline
  MDWLENGTH & double & Characteristic length for Myosin II walking rate change \\
  \hline
