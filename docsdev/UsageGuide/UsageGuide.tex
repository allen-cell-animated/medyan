...
\item \texttt{BOUND} : A bound species on a filament. There must be at least one bound species defined in the system for each type of binding molecule, which will denote the "empty" spot on a filament for that molecule. They must be listed in order of 1) branching binding site, 2) Linker binding site, 3) motor binding site.
