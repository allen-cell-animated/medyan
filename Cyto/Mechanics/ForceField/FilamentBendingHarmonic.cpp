...
double FilamentBendingHarmonic::energy(Bead* b1, Bead* b2, Bead* b3, double k_bend, double d ){
    ...
    double L1 = sqrt(ScalarProductStretched(b1->coordinate, b1->force, b2->coordinate, b2->force, b1->coordinate, b1->force, b2->coordinate, b2->force, d));
    double L2 = sqrt(ScalarProductStretched(b2->coordinate, b2->force, b3->coordinate, b3->force, b2->coordinate, b2->force, b3->coordinate, b3->force, d));
    
    double L1L2 = L1*L2;
    
    double l1l2 = ScalarProductStretched(b1->coordinate, b1->force, b2->coordinate, b2->force, b2->coordinate, b2->force, b3->coordinate, b3->force, d);
...
void FilamentBendingHarmonic::forces(Bead* b1, Bead* b2, Bead* b3, double k_bend ){
    ...
    b2->force[0] +=  k_bend *( (b3->coordinate[0] - 2*b2->coordinate[0] + b1->coordinate[0])*A - (b2->coordinate[0] - b1->coordinate[0])*B + (b3->coordinate[0] - b2->coordinate[0])*C );
    b2->force[1] +=  k_bend *( (b3->coordinate[1] - 2*b2->coordinate[1] + b1->coordinate[1])*A - (b2->coordinate[1] - b1->coordinate[1])*B + (b3->coordinate[1] - b2->coordinate[1])*C );
    b2->force[2] +=  k_bend *( (b3->coordinate[2] - 2*b2->coordinate[2] + b1->coordinate[2])*A - (b2->coordinate[2] - b1->coordinate[2])*B + (b3->coordinate[2] - b2->coordinate[2])*C );
...
void FilamentBendingHarmonic::forcesAux(Bead* b1, Bead* b2, Bead* b3, double k_bend ){
    ...
    //force on i, f = k*(A*(l1-l2) - B*l1 + C*l2):
    b2->forceAux[0] +=  k_bend *( (b3->coordinateAux[0] - 2*b2->coordinateAux[0] + b1->coordinateAux[0])*A - (b2->coordinateAux[0] - b1->coordinateAux[0])*B + (b3->coordinateAux[0] - b2->coordinateAux[0])*C );
    b2->forceAux[1] +=  k_bend *( (b3->coordinateAux[1] - 2*b2->coordinateAux[1] + b1->coordinateAux[1])*A - (b2->coordinateAux[1] - b1->coordinateAux[1])*B + (b3->coordinateAux[1] - b2->coordinateAux[1])*C );
    b2->forceAux[2] +=  k_bend *( (b3->coordinateAux[2] - 2*b2->coordinateAux[2] + b1->coordinateAux[2])*A - (b2->coordinateAux[2] - b1->coordinateAux[2])*B + (b3->coordinateAux[2] - b2->coordinateAux[2])*C );
