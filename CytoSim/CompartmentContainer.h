...
class CompartmentGridKey {friend class SubSystem; friend class SimpleInitializerImpl; CompartmentGridKey(){} ~CompartmentGridKey(){} };
...
private:
    ...
    CompartmentGrid(std::initializer_list<size_t> grid) : _nDim(grid.size()), _prototype_compartment(grid.size()),
                                                          _grid(grid), _is_initialized(false) { assert(0 < _nDim && _nDim <= 3); }
public:
    ...
    static void setInstance(CompartmentGridKey k, std::initializer_list<size_t> grid);
