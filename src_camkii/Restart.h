...
private:
    ...
    void reassignsinglecylfil(bool flag){ //flag 0 - linker/motor, 1-camkiier.
...
//            TODO if(scfmap.size() > SysParams::Chemistry().numBindingSites[x->getType()])
            ...
            if(scfmap.size()){
                ...
                if( nummonomers == SysParams::Geometry().cylinderNumMon[filType]
                    ...
                    auto vecpos = find(camkiicylIDs.begin(), camkiicylIDs.end(), x->getID());
                    //If the cylinder is both a camkii and camkiiing cylinder (like the shaft in letter I), we need to re-do the binding site position when it is camkiiing cylinder.
                    if(vecpos != camkiicylIDs.end() && flag == 1){
                    ...
                    //MAKE SURE THAT YOU ARE NOT MOVING A CAMKII CYLINDER.
                    ...
                    if(abs(mean1-mean2)!= 0 &&  vecpos != camkiicylIDs.end() ){
                        cout<<"Cylinder is not compatible to bind both Link/motor and camkiier. Cannot restart. Exiting."<<endl;
                        //exit(EXIT_FAILURE);
                    }
                    else if(abs(mean1-mean2)!=0 && vecpos == camkiicylIDs.end()){
...
//STEP #1: Get a copy of diffusion rate and set diffusion rate to 0, reset linker motor and camkiiing managers.
                    ...
                    if(filamentType==x->getType()){
                        ...
                        if( angdeltapos.at(0)<0.001 && angdeltapos.at(1)<0.001){
                            ...
                            if(_numMonPerCyl< SysParams::Geometry().cylinderNumMon[filamentType]){
                                if(x->isMinusEnd()){
                                    auto vecpos = find(camkiicylIDs.begin(), camkiicylIDs.end(), x->getID());
                                    if(vecpos!=camkiicylIDs.end()) //If it is a camkii cylinder, then the CMonomers are re-arranged starting from 0 instead of CMonomer.size().
...
//                                        d = round((1-(angdeltapos.at(3)-angdeltapos.at(2))/SysParams::Geometry().cylinderSize[filamentType])*SysParams::Geometry().cylinderNumMon[filamentType]);
    ...
    void addtoHeapcamkiiers(){
 
        vector<tuple<string, short, vector<double>>> camkiiVector=get<2>(filaments);
        int iter;
        auto brows=camkiiVector.size();
        for(auto C : _subSystem->getCompartmentGrid()->getCompartments()) {
            for(auto x : C->getCylinders()) {
                vector<double> b1=x->getFirstBead()->coordinate;
                vector<double> b2=x->getSecondBead()->coordinate;
                ...
                for(iter=0;iter<brows;iter++){
                    ...
                    if(filamentType==x->getType()){
                        ...
                        string boundName=get<0>(b);
                        vector<double> camkii=get<2>(b);
                        //Find the cylinder the camkiier is on
                        vector<double> angdeltapos=getAngleDeltaPos(camkii,b1,b2);
                        if(angdeltapos.at(0)<0.001 && angdeltapos.at(1)<0.001){
                            if(x->getID() == 1629)
                                std::cout<<x->getID()<<endl;
                        ...
                        //Find the closest minus end based on distance alone.
                            ...
                            if(f->getCylinderVector().size()==1){
                                ...
                                camkiicylIDs.push_back(x->getID());
                                ...
                                _bunsortedpairings.insert({iter,make_tuple(x->getCCylinder(),0)});
                                    ...
                                    cout<<"A camkii filament has more than one cylinder and the minus end is not at CMonomer(0). Cannot restart this file. Exiting."<<endl;
                                    //exit(EXIT_FAILURE);
                                }
                            }
                        }
    ...
    void crosscheckCaMKIIers(){
        ...
        for(auto iter=0;iter<brows;iter++){
            ...
            for(int I=0;I<map.size();I++){
                for(int J=I+1;J<map.size();J++){
                    ...
                    if(c1->getID()!=c2->getID() && c1->getType()==filamentType && c2->getType()==filamentType){
                        ...
                        if(check>0){
                            auto distanceproj=twoPointDistance(l1, camkii);
                            if(distanceproj>0.0001)
                            {cout<<"Serious error! CaMKIIer "<<iter<<" binding site does not exist"<<endl; break;}
                            if(check==1)
                                distanceproj=twoPointDistance(camkii, c2->getFirstBead()->coordinate);
                            else if(check==2)
                                distanceproj=twoPointDistance(camkii, c1->getFirstBead()->coordinate);
            ...
            if(!check2)
            {cout<<"Serious error! Bound Species (CaMKIIer) with the following coordinates is not bound to a legitimate site"<<endl;
                cout<<camkii[0]<<" "<<camkii[1]<<" "<<camkii[2]<<endl;}
            auto c11=get<0>(map[one])->getCylinder()->getFirstBead()->coordinate;
            auto c1=get<0>(map[one])->getCylinder();
            auto c21=get<0>(map[two])->getCylinder()->getFirstBead()->coordinate;
            
            for(auto &Mgr:c1->getCompartment()->getFilamentBindingManagers()){
                if(dynamic_cast<CaMKIIingManager*>(Mgr.get())) {
                    if(Mgr->getBoundName().compare(boundName)==0){
