...
//    std::cout<<"C+F---------------------------- "<<endl;
    ...
    cudaMemcpy(U_i, gU_i, blocksnthreads[0]*blocksnthreads[1]*sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(U_ii, gU_ii, blocksnthreads[0] * blocksnthreads[1] * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(c1, gc1, 3*blocksnthreads[1]*sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(c2, gc2, 3*blocksnthreads[1]*sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(checkU, gcheckU, blocksnthreads[1]*sizeof(double), cudaMemcpyDeviceToHost);
...
//    gcheckvar = NULL;
    ...
    double dd[1];
    cudaMemcpy(dd, z, sizeof(double), cudaMemcpyDeviceToHost);
...
//    cudaMemcpy(cforce, f, 3*Bead::getBeads().size()*sizeof(double), cudaMemcpyDeviceToHost);
    int cparams[2];
    cudaMemcpy(cparams, params, 2*sizeof(int), cudaMemcpyDeviceToHost);
//    for(auto i =0; i < Bead::getBeads().size(); i++)
//
//        std::cout<<ccoord[3 * i]<<" "<<ccoord[3 * i +1]<<" "<<ccoord[3 * i +2]<<" "<<cforce[3 * i]<<" "<<cforce[3 * i
//                                                                                                                +1]<<" "<<cforce[3 * i +2]<<endl;
...
//    std::cout<<"Memory "<<freeMem<<" "<<totalMem<<endl;
    ...
    cudaMemcpy(c1, gc1, 3*blocksnthreads[1] * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(c2, gc2, 3*blocksnthreads[1] * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(checkU, gcheckU, blocksnthreads[0] * blocksnthreads[1] * sizeof(double), cudaMemcpyDeviceToHost);
...
void MotorGhostStretchingHarmonic::forces(double *coord, double *f, int *beadSet,
    ...
    double c1[3 * blocksnthreads[0] * blocksnthreads[1]], c2[3 * blocksnthreads[0] * blocksnthreads[1]];
    double F_i[3*blocksnthreads[0] * blocksnthreads[1]];
    ...
    MotorGhostStretchingHarmonicforces<<<blocksnthreads[0], blocksnthreads[1], (12 * blocksnthreads[1]) * sizeof
