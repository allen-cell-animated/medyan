...
void TriangleCylinderBeadExclVolRepulsion::forcesAux(
    Vertex* v0, Vertex* v1, Vertex* v2, Bead* b,
    double area, const Vec3& dArea0, const Vec3& dArea1, const Vec3& dArea2,
    double kExVol
) {
    ...
    for(size_t coordIdx = 0; coordIdx < 3; ++coordIdx) {
        ...
        v0->forceAux[coordIdx] -= kExVol * 2 * (dArea0[coordIdx] * H + dH[0][coordIdx] * area);
        v1->forceAux[coordIdx] -= kExVol * 2 * (dArea1[coordIdx] * H + dH[1][coordIdx] * area);
        v2->forceAux[coordIdx] -= kExVol * 2 * (dArea2[coordIdx] * H + dH[2][coordIdx] * area);
    }
}
