...
template <class BRepulsionInteractionType>
double BoundaryCylinderRepulsionIn<BRepulsionInteractionType>::computeEnergy(double *coord, double *f, double d) {
...
template <class BRepulsionInteractionType>
void BoundaryCylinderRepulsionIn<BRepulsionInteractionType>::computeForces(double *coord, double *f) {
#ifdef CUDAACCL
    //has to be changed to accomodate aux force
    double * gpu_coord=CUDAcommon::getCUDAvars().gpu_coord;
    double * gpu_force;
        ...
        for(auto &c : _neighborList->getNeighbors(be)) {
            ...
            if(c->isMinusEnd()) {
                ...
                for (int i = 0; i < cylSize; i++) {
                    ...
                    // Projection magnitude ratio on the direction of the cylinder
                    // (Effective monomer size) = (monomer size) * proj
                    //ouble proj = -dotProduct(be->normal(newCoord), normal);
                    //if(proj < 0.0) proj = 0.0;
