...
vector<int> GController::getCompartmentIndices(const vector<double>& coords) {
    /**************************************************************************
...
bool GController::indicesOutOfBound(const vector<int>& indices) {
    for(size_t coordIdx = 0; coordIdx < _nDim; ++coordIdx)
        if(indices[coordIdx] < 0 || indices[coordIdx] >= _grid[coordIdx])
            return true;
    return false;
}
