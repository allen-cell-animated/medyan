...
traj_filename = 'snapshot.traj'

color_filename = ''
#color_filename = 'tensions.traj'

#radius=[500,]
#with open(force_filename) as force:
#    for line in force:
#        if len(line)>len('Current Radius = ') and line[:len('Current Radius = ')]=='Current Radius = ':
#            radius+=[float(line[len('Current Radius = '):])]
...
for line in traj_file:
    ...
    if(first_snapshot_line):
        ...
        if len(line.split()) == 8:
            ...
            F.n_motors, F.n_branchers, F.n_camkiiers, F.n_bubbles  = map(float,line.split())
        else:
            F.step, F.time, F.n_filaments, F.n_linkers, \
            F.n_motors, F.n_branchers, F.n_camkiiers = map(float,line.split())

        F.n_filaments = int(F.n_filaments)
        F.n_linkers   = int(F.n_linkers)
        F.n_motors    = int(F.n_motors)
        F.n_branchers = int(F.n_branchers)
        F.n_camkiiers = int(F.n_camkiiers)
    ...
    if(len(line)==0):
        ...
        assert F.n_camkiiers == len(F.camkiiers)
    ...
    if(first_line):
        ...
        if(line_split[0] == "MOTOR") or (line_split[0] == "M"):
            MS = MotorSnapshot()
            MS.id, MS.type = map(int, line_split[1:-1])
            first_line = False
            F.motors[MS.id] = MS
        ...
        if(line_split[0] == "BRANCHER") or (line_split[0] == "B"):
            ...
            line_number+=1
            continue
        if(line_split[0] == "CAMKIIER") or (line_split[0] == "B"):
            BS = CaMKIIerSnapshot()
            BS.id, BS.type = map(int, line_split[1:])
            first_line = False
            F.camkiiers[BS.id] = BS
            n_beads_camkiier+=1
            reading_camkiier=True
            line_number+=1
            continue
...
def implicit_plot(expr, ext_grid, fig_handle=None, Nx=101, Ny=101, Nz=101,
    ...
    if fig_handle==None:  # create a new figure
        fig = mlab.figure(1,bgcolor=(0.1, 0.1, 0.1), fgcolor=(0, 0, 0), size=(800, 800))
    else:
        fig = fig_handle

    xl, xr, yl, yr, zl, zr = ext_grid
    x, y, z = mgrid[xl:xr:eval('{}j'.format(Nx)),
    ...
    if ori_axis:
        len_caxis = int(1.05*max(abs(array(ext_grid))))
...
@mlab.show
def show_snapshot(snapshot_number=-1, orientation=(0,235)):
    ...
    #DIAMETER = radius[snapshot_number+1]*2.0

    #default color, in RGB
    DBEADCOLOR    = (1.0,1.0,0.0) 
    ...
    BRANCHENDCOLOR =(1.0,1.0,0.0)
    CAMKIIENDCOLOR =(0.7,0.3,0.1)

    local_snapshot=TrajSnapshotList[snapshot_number]
    figw = mlab.figure(1, size=(1000, 1000), bgcolor=(0.1,0.1,0.1))
    #figw = mlab.figure(1, size=(1000, 1000), bgcolor=(0.5,0.5,0.5))
    mlab.clf()
    ...
    if BOUNDARYTYPE == "CUBIC" :
        ...
        outline=mlab.pipeline.outline(pts, line_width=0.6, color=(1.0,1.0,1.0))
        ...
        implicit_plot('((x - {i})/ {a})**2 + ((y - {j}) / {b})**2 + ((z - {k})/ {c})**2 - 1'.format(a=GRIDSIZEMAXX/2, b=GRIDSIZEMAXY/2, c=GRIDSIZEMAXZ/2, 
                      ...
                      (0 - 200, GRIDSIZEMAXX + 200, 0 - 200, GRIDSIZEMAXY + 200, 0 - 200, GRIDSIZEMAXZ + 200),
                        fig_handle=figw, Nx=64, Ny=64, Nz=64, col_isurf=(0.67, 0.77, 0.93),
                       opaque=False, opa_val=0.3, ori_axis=False)
    #added for spherical, when time wasn't working
    #display time
    #time = 'Time = %3.1ds'%local_snapshot.time# + "s"
    #print time        
    #global ax    
    #ax=mlab.axes(x_axis_visibility=True,y_axis_visibility=False,z_axis_visibility=False,xlabel=time,nb_labels=2,extent=[0,1000,0,1000,0,1000])
    #ax.axes.font_factor=2
    #global text        
    #text=mlab.text(0.8, 0.98, time,width=0.1)#,color=(0,0.5,1))#, color=(0.5,0.5,0.0),opacity=0,figure=figw)    
    #mlab.view(0,180)

    #display time
    time = 'Time = {0: >.2f}s'.format(float(local_snapshot.time))
    #mlab.text(0.05, 0.9, time, color=(1.0,1.0,1.0),width=0.3)
    mlab.text(0.6, 0.9, time, color=(1.0,1.0,1.0))
    global ax    
    ax=mlab.axes(x_axis_visibility=True,y_axis_visibility=False,z_axis_visibility=False,nb_labels=2,extent=[0,1000,0,1000,0,1000])
    ax.axes.font_factor=2
    
    #mlab.move(right=-5.0)    
    mlab.view(orientation[0],orientation[1])
    #mlab.orientation_axes(color=(1,1,1))
    ...
    if(BOUNDARYTYPE == "CUBIC"):
        ...
        #mlab.points3d(n1_x,n1_y,n1_z, scale_factor=12.0, color=DFILCOLOR) #scale_factor = 12.0
    ...
    if(len(local_snapshot.filaments) != 0):
        ...
        if (COLORBYANGLE): 
            for filamentID, FS in local_snapshot.filaments.iteritems():
                ...
                for i in xrange(1, len(FS.coords[0]), 1):
                    ...
                    b1 = array([b1x, b1y, b1z])
                    b2 = array([b2x, b2y, b2z])

                    #calculate angle from z plane
                    dist = sqrt((b2 - b1).dot(b2 - b1))
                    direction = (b2 - b1) / dist

                    angle = math.acos(direction.dot(array([0,0,1]))) * 180 / math.pi - 90
        ...
        tube=mlab.pipeline.tube(src, tube_radius=5.0)
        ...
        if(len(c) != 0):
            ...
            cb = mlab.colorbar(object=surface, orientation='vertical', 
                                 title=SCALETITLE, label_fmt='%.2f', nb_labels=5)

            cb.title_text_property.color = (1.0,1.0,1.0)        
            cb.label_text_property.color = (1.0,1.0,1.0)
            cb.scalar_bar_representation.position = [0.89, 0.15]
    ...
    if(len(local_snapshot.branchers) != 0):
        ...
        src = mlab.pipeline.scalar_scatter(x[0], x[1], x[2])
        gsphere=mlab.pipeline.glyph(src, mode="sphere", resolution=24, 
                                    scale_factor=20.0, color=BRANCHENDCOLOR)

    #DISPLAYING CaMKII 
    if(len(local_snapshot.camkiiers) != 0):
        x=[]

        for i in 0, 1, 2:
            q=[]
            for bid in sorted(local_snapshot.camkiiers.keys()):
                q.append(local_snapshot.camkiiers[bid].coords[i])
            x.append(hstack(q))

        # Create the points
        src = mlab.pipeline.scalar_scatter(x[0], x[1], x[2])
        gsphere=mlab.pipeline.glyph(src, mode="sphere", resolution=24, 
                                    scale_factor=50.0, color=CAMKIIENDCOLOR)
    ...
    if(saving):
        mlab.savefig(filename=saveFile + "Snapshot" + '%03i'%(snapshot_number) + ".png")
